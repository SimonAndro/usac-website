...
{/if}
{if $logoon == '1'}
<a class="navbar-brand" href="{$sitepath}"><img src="{$sitepath}/themes/{$themes}/styles/images/logo.png" alt="{$sitetitle}" width="250" height="45" /></a>
{/if}
...
{if $rewritemod == 2}
<li class="nav-item"><a style="color:#FFF;" class="nav-link" href="{$sitepath}/categories.php?id={$caty.catid}">{$caty.name|stripslashes}</a>
{/if}
{if $rewritemod == 1}
<li class="nav-item"><a style="color:#FFF;" class="nav-link" href="{$sitepath}/category/{$caty.catid}/{$caty.seoname}.html">{$caty.name|stripslashes}</a>
{/if}
<ul>
{foreach from=$subcat item=inc}
{if $inc.cord neq 0 && $caty.catid eq $inc.parent}
{if $rewritemod == 2}
<li class="nav-item"><a style="color:#FFF;" class="nav-link" href="{$sitepath}/categories.php?id={$inc.catid}">{$inc.name|stripslashes|replace:" ":"&nbsp;"}</a>
{/if}
{if $rewritemod == 1}
<li class="nav-item"><a style="color:#FFF;" class="nav-link" href="{$sitepath}/category/{$inc.catid}/{$inc.seoname}.html">{$inc.name|stripslashes|replace:" ":"&nbsp;"}</a>
{/if}
{/if}
{/foreach}
</ul>
...
{if $rewritemod == 2}
<li class="nav-item"><a style="color:#FFF;" class="nav-link" href="{$sitepath}/categories.php?id={$morecat.catid}">{$morecat.name|stripslashes|replace:" ":"&nbsp;"}</a>
{/if}
{if $rewritemod == 1}
<li class="nav-item"><a style="color:#FFF;" class="nav-link" href="{$sitepath}/category/{$morecat.catid}/{$morecat.seoname}.html">{$morecat.name|stripslashes|replace:" ":"&nbsp;"}</a>
{/if}
{/if}
{foreachelse}
<li class="nav-item"><a  style="color:#FFF;" class="nav-link" href="{$sitepath}">Categories</li></a>
{/foreach}
</ul>
{/if}
</ul>
...
<input name="q" class="form-control my-0 py-1 amber-border" type="text" placeholder="{$lang.298}" aria-label="Search"{if $smarty.get.q|default eq false}{else} value="{$smarty.get.q|escape|default}"{/if} />
...
{if $newson eq 2}<div class="container mt-3"><div class="row"><div class="col-md-12 text-center"><div class="alert alert-success">{$newstext}</div></div></div></div>{/if}